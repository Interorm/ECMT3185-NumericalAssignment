...
def Pertubate(params:dict, params_info:dict) -> dict:
    ...
    for param, old_value in params.items():
        ...
        step_size = params_info[param]['step_size']
...
def Acceptance(old_rsme, new_rsme, temperature):
    
    accepting_probability = min(1, np.exp( (old_rsme - new_rsme) / (old_rsme * temperature) ))
...
def TestModel(X, Y, model, fixed_params:dict, test_params:dict):
    ...
    estimator = model(**fixed_params, **test_params)
    rsme = np.mean( -cross_val_score(estimator, X, Y, n_jobs=-1,
...
def MCMC_Tuning(X, Y, model,
                ...
                temperature_start:float, target_acceptance_start:float, 
                temperature_decay:float=0.998, target_acceptance_decay:float=0.998, 
                adaption_schedule:int=10, 
                autosave:bool=False, load=None, load_burnin=None,
                debug:bool=False, visualize:bool=False):
    ...
    if load is None and load_burnin is None:
    ...
    elif load_burnin is not None:
        print('Load Burn-In...')
        with open(load_burnin, 'rb') as f: burnin_chains = pickle.load(f)
        
        chains = [None] * chain_number
        for i, chain in enumerate(chains):
            chains[i] = {
                'samples':[burnin_chains[i]['samples'][-1]],
                'acceptance_history':[],
                'temperature':[temperature_start]
            }
        start = 1
    ...
    if start == 1: target_acceptance = target_acceptance_start
    else:          target_acceptance = target_acceptance_start * (target_acceptance_decay ** np.floor(start/adaption_schedule))


    print('Starting MCMC Algorithm...')
    if visualize: 
        plt.ion()
        n_plots = len(info_params) + 2
        n_rows = int(np.ceil(n_plots/2))
        fig, axes = plt.subplots(n_rows, 2, figsize=(14, 4 * n_rows))
        plt.tight_layout()

        axes = axes.flatten()
        param_names = list(info_params.keys())
        ...
        for ax, param in zip(axes[:-2], param_names):
            ...
            for i in range(chain_number):
                ...
                lines[param].append(line)

        # RSME
        axes[-2].set_title("rsme")
        lines['rsme'] = []
        for i in range(chain_number):
            line, = axes[-2].plot([], [], alpha=0.8)
            lines['rsme'].append(line)

        # Acceptance
        axes[-1].set_title("Average Acceptance Rate")
        lines['acceptance'] = []
        for i in range(chain_number):
            line, = axes[-1].plot([], [], alpha=0.8)
            lines['acceptance'].append(line)
    ...
    for iteration in tqdm(range(start,chain_length), total=chain_length-start+1,
        ...
        for chain in tqdm(chains,
            ...
            if debug:
                ...
                old_rsme = old_proposal['rsme']
                new_rsme = new_proposal['rsme']
                print(f'Old RSME: {old_rsme}')
                print(f'New RSME: {new_rsme}')
            ...
            if iteration % adaption_schedule == 0 and iteration > 0:
                ...
                chain['temperature'][-1] *= temperature_decay
                target_acceptance *=  target_acceptance_decay
                ...
                if accepance_rate > target_acceptance + 0.1666:
                    chain['temperature'][-1] *= (1 / 1.05)

                #Autosave for on-th-fly changes and tests
                if autosave:
                    with open(f'models/MCMC_{model.__name__}_Autosave_v2.sav', 'wb') as f: pickle.dump(chains, f)
                ...
                if visualize:
                    for c_idx, chain in enumerate(chains):
                        ...
                        for param in param_names:
                            y_data = [s['params'][param] for s in samples]
                            lines[param][c_idx].set_data(range(len(y_data)), y_data)

                        # Update RSME
                        rsme_data = [s['rsme'] for s in samples]
                        lines['rsme'][c_idx].set_data(range(len(rsme_data)), rsme_data)

                        # Update acceptance
                        acc_data = np.cumsum(chain['acceptance_history']) / np.arange(1, len(chain['acceptance_history'])+1)
                        lines['acceptance'][c_idx].set_data(range(len(acc_data)), acc_data)
    ...
    with open(f'models/MCMC_{model.__name__}_v2.sav', 'wb') as f: pickle.dump(chains, f)
   ...
   # Load Data and Prepare for Processesing (PrepX)
    ...
    if XGBBoost:
        ...
        MCMC_XGBoost = MCMC_Tuning(X_DATA, Y_DATA, xgb.XGBRegressor, fixed_params, test_params,
                                   2000, 2500, 10, 0.25, 0.6, autosave=True, visualize=False, load_burnin='models/MCMC_XGBoost.sav')
